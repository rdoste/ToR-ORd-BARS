...
function currents = getCurrentsStructure_BARS(time, X, parameters, ignoreFirstSpikes)
...
radiusmultiplier = 1; if (isfield(parameters,'radiusmultiplier')) radiusmultiplier = parameters.radiusmultiplier; end
...
currents.fICaLP = zeros(nPoints, 1);
currents.fIKsP = zeros(nPoints, 1);
currents.fPLBP = zeros(nPoints, 1);
currents.fTnIP = zeros(nPoints, 1);
currents.fINaP = zeros(nPoints, 1);
currents.fINaKP = zeros(nPoints, 1);
currents.fRyRP = zeros(nPoints, 1);
currents.fIKurP = zeros(nPoints, 1);
...
for iBeat = 1:size(time, 1)
    ...
    Xhere = X{iBeat};
     ...
     extraParams.ISO=parameters.ISO(iBeat);
     extraParams.runSignalingPathway=parameters.runSignalingPathway(iBeat);
     extraParams.const_signaling= Constants_SignalingMyokit2(extraParams.ISO,radiusmultiplier); 
         ...
         for j=1:size(X{iBeat},1)
           ...
           IsJs=modelNow(time{iBeat}(j),X{iBeat}(j,:),0, cellType, ICaL_Multiplier, ...
            ...
            currents.nai(i) = X{iBeat}(j,2);
            currents.nass(i) = X{iBeat}(j,3);
            currents.ki(i) = X{iBeat}(j,4);
            currents.kss(i) = X{iBeat}(j,5);
   
             if length(IsJs)>36

                currents.fICaLP(i) = IsJs(37);
                currents.fIKsP(i) = IsJs(38);
                currents.fPLBP(i) = IsJs(39);     
                currents.fTnIP(i) = IsJs(40); 
                currents.fINaP(i) = IsJs(41); 
                currents.fINaKP(i) = IsJs(42);
                currents.fRyRP(i) = IsJs(43);
                currents.fIKurP(i) = IsJs(44);
                currents.Ta(i) = IsJs(45);
                %currents.fJupp_PLB(i) = IsJs(45);
