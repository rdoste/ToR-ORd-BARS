...
%     02-2022: Added compatibility with beta-adrenergic stimulation
% 
...
function output=model_Torord_dynCl_BARS(t,X,flag_ode, cellType, ICaL_Multiplier, ...
...
%      dPss=1.0763*exp(-1.0070*exp(-0.0829*(v+9.5)));  % %BetaAdrenergic;
     dPss=1.0323*exp(-1.0553*exp(-0.0810*(v+9.5)));     %BetaAdrenergic ,fitted curve
    dPss=min(dPss,1);
    dd_P=(dPss-d_P)/td;
%   fss_P=1.0/(1.0+exp((v+19.58+8.0)/3.696)); %BetaAdrenergic; 
  ...
  %% The rest (PKA)
PCa_P=8.3757e-05 * ICaL_PCaMultiplier * 1.1;  %BetaAdrenergic;  (%Gong--> 1.9)
if celltype==1
    PCa_P=PCa_P*1.2;
elseif celltype==2
    PCa_P=PCa_P*2;
...
Knai0_np = 9.073;
Knai0_P = Knai0_np*0.7;%BetaAdrenergic;
Knai0=9.073;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fINaK_PKA = fINaKP;
if fINaK_PKA == 0
...
Jrel= Jrel_Multiplier* 1.5378* ((1.0-fJrel_CaMKonly-fJrel_PKAonly-fJrel_BP)*Jrelnp + fJrel_CaMKonly*Jrelp + fJrel_PKAonly*Jrelnp_P + fJrel_BP*Jrelp_P );
